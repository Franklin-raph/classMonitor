...
const studentProfileUpdate = async (req, res) => {
  ...
  try {
    ...
    Student.findOne({ studentID: req.params.student_id }).then((signedInStudent) => {
      ...
      signedInStudent.github = req.body.github;
...
const forgotPassword = async (req, res) => {
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      pass: process.env.CLASS_MONITOR_APP_PASSWORD,
    },
  });
...
const updateStudentPassword = async (req, res) => {
  ...
  try {
    ...
    await Student.findOne({ studentID: req.params.student_id }).then(async (signedInStudent) => {
      console.log(signedInStudent);
